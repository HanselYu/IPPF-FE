...
def Load_data():
    print('Data Loading...')
    Sequence = []
    with open('Dipeptidyl_peptidase‑IV_inhibitory_peptides/train-positive.fasta', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    with open('Dipeptidyl_peptidase‑IV_inhibitory_peptides/train-negative.fasta', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    with open('Dipeptidyl_peptidase‑IV_inhibitory_peptides/test-positive.fasta', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    with open('Dipeptidyl_peptidase‑IV_inhibitory_peptides/test-negative.fasta', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    Mysequence = []
...
def Peptide_predict(features_crafted, features_normalize):
    ...
    Label = np.concatenate((np.ones([532], dtype=int), np.zeros([532], dtype=int)), axis=0)
    ...
    res.to_excel('Peptide_bitter.xlsx')


def Peptide_independent(features_crafted, features_normalize):
    features_ensemble = np.concatenate((features_normalize, features_crafted), axis=1)
    Label = np.concatenate((np.ones([532], dtype=int), np.zeros([532], dtype=int),
                            np.ones([133], dtype=int), np.zeros([133], dtype=int)), axis=0)
    model = ExtraTreesClassifier()
    Train_data = features_ensemble[:1064]
    Test_data = features_ensemble[1064:]
    Train_label = Label[:1064]
    Test_label = Label[1064:]
    model.fit(Train_data, Train_label)
    Pre_label = model.predict(Test_data)
    Acc = metrics.accuracy_score(Test_label, Pre_label)
    MCC = metrics.matthews_corrcoef(Test_label, Pre_label)
    ...
    plt.title("DPP_IV receiver operating characteristic")
    plt.legend(loc="lower right")
    plt.savefig('PDPP_IV.jpg')
    Spec = CM[0][0] / (CM[0][0] + CM[0][1])
    Sens = CM[1][1] / (CM[1][0] + CM[1][1])
    print('Accuracy:', Acc, " Sensitivity", Sens, " Specificity", Spec, "MCC", MCC, "auROC", auROC)
...
def Write_features(features_crafted, features_normalize, min_length):
    ...
    res.to_excel('Features_DPPIV.xlsx')


if __name__ == '__main__':
    Sequence, Mysequence = Load_data()
    ###### Train
    # Sequence, Mysequence = Sequence[:512], Mysequence[:512]
