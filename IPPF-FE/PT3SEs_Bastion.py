...
def Load_data():
    print('Data Loading...')
    Sequence = []
    with open('T3SEs/train/train_pos.txt', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    with open('T3SEs/train/train_neg.txt', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    with open('T3SEs/test/test_pos.txt', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    with open('T3SEs/test/test_neg.txt', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    with open('T3SEs/indel/Baston3_T3_DEP_POS_108.txt', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    with open('T3SEs/indel/Baston3_T3_DEP_NEG_108.txt', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    # sequence truncation
...
def Peptide_predict(features_crafted, features_normalize):
    ...
    Label = np.concatenate((np.ones([303], dtype=int), np.zeros([604], dtype=int),
                            np.ones([76], dtype=int), np.zeros([151], dtype=int)), axis=0)
    ...
    res.to_excel('Protein_T3SEs2_Bastion.xlsx')


def Peptide_independent(features_crafted, features_normalize):
    features_ensemble = np.concatenate((features_normalize, features_crafted), axis=1)
    Label = np.concatenate((np.ones([303], dtype=int), np.zeros([604], dtype=int),
                            np.ones([76], dtype=int), np.zeros([151], dtype=int),
                            np.ones([108], dtype=int), np.zeros([108], dtype=int)), axis=0)
    model = xgboost.XGBClassifier()
    Train_data = features_ensemble[:1134]
    Test_data = features_ensemble[1134:]
    Train_label = Label[:1134]
    Test_label = Label[1134:]
    model.fit(Train_data, Train_label)
    Pre_label = model.predict(Test_data)
    Acc = metrics.accuracy_score(Test_label, Pre_label)
    MCC = metrics.matthews_corrcoef(Test_label, Pre_label)
    ...
    plt.title("Type III secreted effector_2 receiver operating characteristic")
    plt.legend(loc="lower right")
    plt.savefig('PT3SEs_2.jpg')
    Spec = CM[0][0] / (CM[0][0] + CM[0][1])
    Sens = CM[1][1] / (CM[1][0] + CM[1][1])
    print('Accuracy:', Acc, " Sensitivity", Sens, " Specificity", Spec, "MCC", MCC, "auROC", auROC)
...
def Write_features(features_crafted, features_normalize, min_length):
    ...
    res.to_excel('Features_T3SEs2_Bastion.xlsx')


if __name__ == '__main__':
    Sequence, Mysequence = Load_data()
    ###### Train
    # Sequence, Mysequence = Sequence[:1134], Mysequence[:1134]
